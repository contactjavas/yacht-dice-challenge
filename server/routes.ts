...
export async function registerRoutes(app: Express): Promise<Server> {
	...
	app.post("/api/games/:code/join", async (req, res) => {
		try {
			...
			if (!gameState) {
				return res
					.status(404)
					.json({ message: "Game not found or cannot be joined" });
			}
	...
	wss.on("connection", (ws: WebSocket, req) => {
		...
		try {
			...
			console.error(
				`WS [${connectionId}] Error sending initial connection message:`,
				error,
			);
		}
			...
			if (ws.readyState === 1) {
				try {
					...
					if (timeSinceLastPing > 60000 && playerId && gameId) {
						console.log(
							`WS [${connectionId}] Connection appears stale, no activity for ${timeSinceLastPing}ms`,
						);
					}
			...
			try {
				...
				if (data.type === "CLIENT_PING") {
					try {
						...
						console.error(
							`WS [${connectionId}] Error sending pong response:`,
							error,
						);
					}
				}
				...
				if (data.action === "REGISTER_PLAYER") {
					...
					console.log(`WS [${connectionId}] registering player:`, {
						playerId,
						gameId,
					});
					...
					if (playerId && gameId) {
						...
						console.log(
							`WS [${connectionId}] player ${playerId} registered for game ${gameId}`,
						);
						...
						if (gameState) {
							console.log(
								`WS [${connectionId}] sending initial game state to player ${playerId}`,
							);
							ws.send(
								JSON.stringify({
									type: "GAME_UPDATE",
							...
							console.warn(
								`WS [${connectionId}] game state not found for game ${gameId}`,
							);
						}
					} else {
						console.error(`WS [${connectionId}] invalid player registration:`, {
							playerId,
							gameId,
						});
					}
				...
				if (!playerId || !gameId) {
					console.error(
						`WS [${connectionId}] unauthenticated action:`,
						data.action,
					);
					ws.send(
						JSON.stringify({
				...
				switch (data.action) {
					case "START_GAME":
						if (data.data?.hostId) {
							console.log(
								`WS [${connectionId}] starting game ${gameId} by host ${data.data.hostId}`,
							);
							const gameState = await gameManager.startGame(
								gameId,
								data.data.hostId,
							);
							if (gameState) {
								...
								console.error(
									`WS [${connectionId}] failed to start game ${gameId}`,
								);
							}
						} else {
							console.error(
								`WS [${connectionId}] missing hostId for START_GAME action`,
							);
						}
						break;
					...
					case "ROLL_DICE":
						if (data.data?.selectedDice) {
							console.log(
								`WS [${connectionId}] player ${playerId} rolling dice in game ${gameId}`,
							);
							const gameState = await gameManager.rollDice(
								gameId,
								playerId,
								data.data.selectedDice,
							);
							if (gameState) {
								gameManager.broadcastGameUpdate(gameId);
							} else {
								console.error(
									`WS [${connectionId}] dice roll failed for player ${playerId} in game ${gameId}`,
								);
							}
						} else {
							console.error(
								`WS [${connectionId}] missing selectedDice for ROLL_DICE action`,
							);
						}
						break;
					...
					case "SELECT_DICE":
						if (data.data?.diceIndices) {
							console.log(
								`WS [${connectionId}] player ${playerId} selecting dice in game ${gameId}`,
							);
							const gameState = await gameManager.selectDice(
								gameId,
								playerId,
								data.data.diceIndices,
							);
							if (gameState) {
								gameManager.broadcastGameUpdate(gameId);
							} else {
								console.error(
									`WS [${connectionId}] dice selection failed for player ${playerId} in game ${gameId}`,
								);
							}
						} else {
							console.error(
								`WS [${connectionId}] missing diceIndices for SELECT_DICE action`,
							);
						}
						break;
					...
					case "SCORE_CATEGORY":
						if (data.data?.category) {
							console.log(
								`WS [${connectionId}] player ${playerId} scoring category ${data.data.category} in game ${gameId}`,
							);
							const gameState = await gameManager.scoreCategory(
								gameId,
								playerId,
								data.data.category,
							);
							if (gameState) {
								...
								console.error(
									`WS [${connectionId}] category scoring failed for player ${playerId} in game ${gameId}`,
								);
							}
						} else {
							console.error(
								`WS [${connectionId}] missing category for SCORE_CATEGORY action`,
							);
						}
						break;
					...
					case "PASS_TURN":
						console.log(
							`WS [${connectionId}] player ${playerId} passing turn in game ${gameId}`,
						);
						...
						if (gameState) {
							...
							console.error(
								`WS [${connectionId}] pass turn failed for player ${playerId} in game ${gameId}`,
							);
						}
						break;
					...
					case "TOGGLE_READY":
						console.log(
							`WS [${connectionId}] player ${playerId} toggling ready status in game ${gameId}`,
						);
						const updatedState = await gameManager.togglePlayerReady(
							gameId,
							playerId,
						);
						if (updatedState) {
							...
							console.error(
								`WS [${connectionId}] toggle ready failed for player ${playerId} in game ${gameId}`,
							);
						}
						break;
					...
					case "RESTART_GAME":
						if (data.data?.hostId) {
							console.log(
								`WS [${connectionId}] restarting game ${gameId} by host ${data.data.hostId}`,
							);
							const gameState = await gameManager.restartGame(
								gameId,
								data.data.hostId,
							);
							if (gameState) {
								...
								console.error(
									`WS [${connectionId}] failed to restart game ${gameId}`,
								);
							}
						} else {
							console.error(
								`WS [${connectionId}] missing hostId for RESTART_GAME action`,
							);
						}
						break;
				...
				try {
					...
					console.error(
						`WS [${connectionId}] failed to send error response:`,
						e,
					);
				}
			}
		...
		ws.on("close", (code, reason) => {
			console.log(
				`WS [${connectionId}] connection closed with code ${code}`,
