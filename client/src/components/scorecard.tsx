...
const categoryInfo: Record<ScoreCategory, { name: string, description: string }> = {
  ...
  [ScoreCategory.THREE_OF_A_KIND]: { name: 'Three of a Kind', description: 'Sum of all dice if 3+ of one value' },
  [ScoreCategory.FOUR_OF_A_KIND]: { name: 'Four of a Kind', description: 'Sum of all dice if 4+ of one value' },
  [ScoreCategory.FULL_HOUSE]: { name: 'Full House', description: '3 of one value and 2 of another' },
  [ScoreCategory.SMALL_STRAIGHT]: { name: 'Small Straight', description: '4 sequential dice' },
  [ScoreCategory.LARGE_STRAIGHT]: { name: 'Large Straight', description: '5 sequential dice' },
...
export default function Scorecard({ currentDice, scoreCard, isMyTurn, onSelectCategory }: ScoreCardProps) {
  ...
  const totalScore = Object.values(ScoreCategory)
  ...
  const renderCategoryRow = (category: ScoreCategory) => {
    ...
    const potentialScore = isScored ? null : calculatePotentialScore(currentDice, category);
    const isHighPotential = potentialScore && 
    ...
    return (
      <tr 
        ...
        className={`border-b border-neutral-700/50 ${
          isInteractive ? 'hover:bg-neutral-700/30 transition-colors cursor-pointer' : ''
      ...
      >
        <td className="py-2.5 px-3 font-medium">{categoryInfo[category].name}</td>
        <td className="py-2.5 px-3 text-center text-neutral-400">{categoryInfo[category].description}</td>
  ...
  return (
    <div className="bg-neutral-800/60 rounded-xl p-4 md:p-6 overflow-x-auto">
          ...
          <tbody>
            ...
            <tr className="bg-neutral-700/20">
              ...
              <td className="py-2 px-3 text-center font-medium">
                {upperSectionTotal}/63 {upperSectionBonus > 0 && `(+${upperSectionBonus})`}
              </td>
            </tr>
            ...
            <tr className="bg-neutral-700/30">
              <td colSpan={2} className="py-2.5 px-3 font-medium text-lg">Total Score</td>
              <td className="py-2.5 px-3 text-center font-bold text-lg font-['Montserrat']">{totalScore}</td>
            </tr>
          </tbody>
        </table>
      </div>
