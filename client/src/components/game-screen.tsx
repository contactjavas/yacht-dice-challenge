...
export default function GameScreen({
        ...
        // Using refs to track state without triggering re-renders
        const playerRegisteredRef = useRef(false);
        ...
        useEffect(() => {
                ...
                const registerPlayer = () => {
                        try {
                                ...
                                try {
                                        ...
                                        // Use refs for more stable values between renders
                                        const gameId = game?.id || gameIdRef.current;
                                        const currentPlayerId = playerId || playerIdRef.current;
                                        
                                        if (!gameId || !currentPlayerId) {
                                                console.warn("Cannot register player - missing game ID or player ID");
                                                return;
                                        }
                ...
                // Update refs to track current values without causing re-renders
                if (game) gameIdRef.current = game.id;
                if (playerId) playerIdRef.current = playerId;

                // Return a cleanup function to remove event listeners when component unmounts
                return removeListeners;
        }, [socket]); // Only depend on socket to prevent infinite loops
